document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateStep1() {
        ...
        requiredInputs.forEach(input => {
            ...
            if (input.value.trim() === "") {
                ...
                // Ajustado para buscar o rótulo corretamente, se existir
    ...
    function validateStep2() {
        ...
        localStorage.setItem('finalOrder', JSON.stringify(finalOrderData)); // Usando 'finalOrder' agora
        return true;
    }
    ...
    document.getElementById('payment-form').addEventListener('submit', (e) => {
        ...
        if (validateStep2()) {
            ...
            const finalOrderDetails = JSON.parse(localStorage.getItem('finalOrder')); 
    ...
    function renderStep(step, orderData = null) {
        ...
        if (step === 2) {
            ...
            const totalFormatted = formatCurrency(PRODUCT_PRICE);

            orderSummaryEl.innerHTML = `
                <p class="summary-title">Resumo do Pedido</p>
                <p>Produto: <strong>${productTitle}</strong></p>
                <p class="summary-total">Total a Pagar: ${totalFormatted}</p>
            `;
        }
        
        if (step === 3) {
            const dataToUse = orderData || { title: PRODUCT_TITLE, total: PRODUCT_PRICE, paymentMethod: 'cartao' }; // Fallback para cartao
            ...
            const productTitle = dataToUse.title;
